    ...
    <div class="min-h-screen bg-gray-100 py-12 flex flex-col items-center">
        <div class="bg-white shadow-lg sm:rounded-3xl px-8 py-12 max-w-2xl w-full">
            <div class="text-center">
                <h1 class="text-3xl font-bold text-gray-800">Rate the User</h1>
                <p class="text-gray-500 mt-2">Please rate the user for the ride from {{ $offer->location_one }} to {{ $offer->city_one }}.</p>
            </div>

            <form action="{{ route('offers.rateUser.submit', $offer->id) }}" method="POST" class="mt-8">
                @csrf
